#Code for computing Hessian at scale
#Taken from https://github.com/AnonymousNIPS2019/DeepnetHessian
...
class Hessian:
    ...
    def _apply_covar(self, v):
        covar_vec_prod = self.my_zero()
        tot_count = 0
        for i, data in enumerate(self.loader, 0):
            inputs, labels = data[0].to(self.device), data[1].to(self.device)
            tot_count += inputs.shape[0]
            outputs = self.model(inputs)
            loss = self.criterion(outputs, labels)
            z = torch.randn(outputs.shape)
            z.requires_grad = True
            z = z.to(self.device)
            df_dtheta_z = torch.autograd.grad(outputs, self.model.parameters(), z, create_graph=True)
            df_dtheta_vec = torch.autograd.grad(df_dtheta_z, z, v)
            dl_df = torch.autograd.grad(loss, outputs)
            dl_dtheta_vec = torch.sum(dl_df[0] * df_dtheta_vec[0], axis=1, keepdim=True)
            dl_dtheta = torch.autograd.grad(outputs, self.model.parameters(), dl_df[0] * dl_dtheta_vec)
            covar_vec_prod = self.my_sum(dl_dtheta, covar_vec_prod)

        v2 = torch.cat([g.contiguous().view(-1) for g in v])
        proj = torch.dot(self.grad.detach(), v2) * self.grad.detach()
        vec_new = []
        curr_index = 0
        for param in self.model.parameters():
            n = param.nelement()
            vec_new.append(proj[curr_index:curr_index + n].view(param.shape))
            curr_index += n

        covar_vec_prod =  self.my_sub(self.my_div_const(covar_vec_prod, tot_count), vec_new)
        return covar_vec_prod

    def _apply_covar_residual(self, v):
        covar_vec_prod = self._apply_covar(v)
        Hessian_prod = self._apply_Hessian(v)
        v2 = torch.cat([g.contiguous().view(-1) for g in v])
        proj = torch.dot(self.grad.detach(), v2) * self.grad.detach()
        vec_new = []
        curr_index = 0
        ...
        second_mom_vec_prod = self.my_sum(covar_vec_prod, vec_new)
        return self.my_sub(Hessian_prod, second_mom_vec_prod)

    # computes matrix vector multiplication
    # where the matrix is either the Hessian, G or H
    def Hv(self, v):
        if self.hessian_type == 'Hessian':
            return self._apply_Hessian(v)
        elif self.hessian_type == 'G':
            return self._apply_G(v)
        elif self.hessian_type == 'H':
            return self._apply_H(v)
        elif self.hessian_type == 'covar':
            return self._apply_covar(v)
        elif self.hessian_type == 'covar_residual':
            return self._apply_covar_residual(v)
    ...
    def compute_G_decomp(self):
        ...
        for c in tqdm(range(C ** 2)):
            for c_ in range(C ** 2):
                delta_ccp_T_delta_ccp[c, c_] = self.my_inner(delta_ccp_flat[c], delta_ccp_flat[c_]) * prob_ccp_flat[
                    c] / examples_per_class / C
